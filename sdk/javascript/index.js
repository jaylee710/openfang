/**
 * @omtae/sdk — Official JavaScript client for the OMTAE Agent OS REST API.
 *
 * Usage:
 *   const { OMTAE } = require("@omtae/sdk");
 *   const client = new OMTAE("http://localhost:3000");
 *
...
class OMTAEError extends Error {
  constructor(message, status, body) {
    super(message);
    this.name = "OMTAEError";
    this.status = status;
    this.body = body;
  }
}
...
class OMTAE {
  /**
   * @param {string} baseUrl - OMTAE server URL (e.g. "http://localhost:3000")
   * @param {object} [opts]
  ...
  async _request(method, path, body) {
    ...
    if (!res.ok) {
      ...
      throw new OMTAEError("HTTP " + res.status + ": " + text, res.status, text);
    }
...
class AgentResource {
  ...
  async upload(id, file, filename) {
    ...
    if (!res.ok) throw new OMTAEError("Upload failed: " + res.status, res.status);
    return res.json();
  }
...
module.exports = { OMTAE: OMTAE, OMTAEError: OMTAEError };
