# OMTAE installer for Windows
# Usage: iwr -useb https://omtae.sh/install.ps1 | iex
#   or:  powershell -c "irm https://omtae.sh/install.ps1 | iex"
#
...
$DefaultInstallDir = Join-Path $env:USERPROFILE ".omtae\bin"
$InstallDir = if ($env:OPENFANG_INSTALL_DIR) { $env:OPENFANG_INSTALL_DIR } else { $DefaultInstallDir }

function Write-Banner {
    Write-Host ""
    Write-Host "  OMTAE Installer" -ForegroundColor Cyan
...
function Get-Architecture {
    ...
    switch ($archUpper) {
        ...
        default {
            ...
            Write-Host "  Try: cargo install --git https://github.com/RightNow-AI/omtae omtae-cli" -ForegroundColor Yellow
            exit 1
        }
    }
}
...
function Get-LatestVersion {
    ...
    try {
        ...
        Write-Host "    cargo install --git https://github.com/$Repo omtae-cli"
        exit 1
    }
}

function Install-OMTAE {
    Write-Banner

    $arch = Get-Architecture
    $version = Get-LatestVersion
    $target = "${arch}-pc-windows-msvc"
    $archive = "omtae-${target}.zip"
    $url = "https://github.com/$Repo/releases/download/$version/$archive"
    $checksumUrl = "$url.sha256"

    Write-Host "  Installing OMTAE $version for $target..."
    ...
    # Download to temp
    $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "omtae-install"
    ...
    $exePath = Join-Path $tempDir "omtae.exe"
    if (-not (Test-Path $exePath)) {
        # May be nested in a directory
        $found = Get-ChildItem -Path $tempDir -Filter "omtae.exe" -Recurse | Select-Object -First 1
        if ($found) {
            $exePath = $found.FullName
        }
        else {
            Write-Host "  Could not find omtae.exe in archive." -ForegroundColor Red
            Remove-Item -Recurse -Force $tempDir -ErrorAction SilentlyContinue
            exit 1
        }
    }
    ...
    # Install
    Copy-Item -Path $exePath -Destination (Join-Path $InstallDir "omtae.exe") -Force
    ...
    # Verify
    $installedExe = Join-Path $InstallDir "omtae.exe"
    if (Test-Path $installedExe) {
        try {
            $versionOutput = & $installedExe --version 2>&1
            Write-Host ""
            Write-Host "  OMTAE installed successfully! ($versionOutput)" -ForegroundColor Green
        }
        catch {
            Write-Host ""
            Write-Host "  OMTAE binary installed to $installedExe" -ForegroundColor Green
        }
    }

    Write-Host ""
    Write-Host "  Get started:" -ForegroundColor Cyan
    Write-Host "    omtae init"
    Write-Host ""
