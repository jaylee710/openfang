...
fn gateway_dir() -> PathBuf {
    omtae_home().join("whatsapp-gateway")
}
...
pub async fn start_whatsapp_gateway(kernel: &Arc<super::kernel::OMTAEKernel>) {
    ...
    let omtae_url = format!("http://{api_listen}");
...
mod tests {
    ...
    fn test_embedded_files_not_empty() {
        ...
        assert!(GATEWAY_PACKAGE_JSON.contains("@omtae/whatsapp-gateway"));
    }
    ...
    #[test]
    fn test_gateway_dir_under_omtae_home() {
        let dir = gateway_dir();
    ...
    fn test_write_if_changed_creates_new_file() {
        let tmp = std::env::temp_dir().join("omtae_test_gateway");
