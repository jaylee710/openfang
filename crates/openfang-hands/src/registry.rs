...
impl HandRegistry {
    ...
    /// install where the user has not run `omtae hand install` yet.
    ///
    /// Added for issue #984 — custom hands installed via `omtae hand
...
"#;
    ...
    ///
    /// Simulates what `omtae hand config <id> --set KEY=VAL` does against
    ...
    fn test_hand_config_round_trip_via_registry() {
        ...
        cfg.insert(
            ...
            serde_json::Value::String("omtae/1".into()),
        );
        ...
        assert_eq!(
            ...
            Some(&serde_json::Value::String("omtae/1".into()))
        );
