...
cargo tauri signer generate -w ~/.tauri/omtae.key
```
...
Your private key was saved to: ~/.tauri/omtae.key
```
...
Open `crates/omtae-desktop/tauri.conf.json` and replace:
...
|---|---|---|
| `TAURI_SIGNING_PRIVATE_KEY` | Contents of `~/.tauri/omtae.key` | Yes |
...
The following icon files must exist in `crates/omtae-desktop/icons/`:
...
## 5. Set Up the `omtae.sh` Domain

**Status:** BLOCKING for install scripts — users run `curl -sSf https://omtae.sh | sh`.

Options:
- **GitHub Pages**: Point `omtae.sh` to a GitHub Pages site that redirects `/` to `scripts/install.sh` and `/install.ps1` to `scripts/install.ps1` from the repo's latest release.
- **Cloudflare Workers / Vercel**: Serve the install scripts with proper `Content-Type: text/plain` headers.
- **Raw GitHub redirect**: Use `omtae.sh` as a CNAME to `raw.githubusercontent.com/RightNow-AI/omtae/main/scripts/install.sh` (less reliable).

The install scripts reference:
- `https://omtae.sh` → serves `scripts/install.sh`
- `https://omtae.sh/install.ps1` → serves `scripts/install.ps1`

Until the domain is set up, users can install via:
```bash
curl -sSf https://raw.githubusercontent.com/RightNow-AI/omtae/main/scripts/install.sh | sh
```
...
```bash
docker build -t omtae:local .
docker run --rm omtae:local --version
docker run --rm -p 4200:4200 -v omtae-data:/data omtae:local start
```
...
grep '"version"' crates/omtae-desktop/tauri.conf.json
...
Visit: `https://github.com/RightNow-AI/omtae/releases/latest/download/latest.json`
...
docker pull ghcr.io/RightNow-AI/omtae:0.1.0

# Verify both architectures
docker run --rm ghcr.io/RightNow-AI/omtae:latest --version
```
...
4. Open the v0.1.0 app — after 10 seconds it should:
   - Show "OMTAE Updating..." notification
...
omtae --version  # Should print v0.1.0
