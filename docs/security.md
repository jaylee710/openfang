...
system in the OMTAE Agent Operating System.  All struct names, function
...
OMTAE implements **defense-in-depth** security.  No single mechanism is
...
|---|--------|-------|------------------|
| 1 | Capability-Based Security | `omtae-types` | Unauthorized actions by agents |
| 2 | WASM Dual Metering | `omtae-runtime` | Infinite loops, CPU DoS |
| 3 | Merkle Audit Trail | `omtae-runtime` | Tampered audit logs |
| 4 | Taint Tracking | `omtae-types` | Prompt injection, data exfiltration |
| 5 | Ed25519 Manifest Signing | `omtae-types` | Supply chain attacks |
| 6 | SSRF Protection | `omtae-runtime` | Server-Side Request Forgery |
| 7 | Secret Zeroization | `omtae-runtime`, `omtae-channels` | Memory forensics, key leakage |
| 8 | OFP Mutual Auth | `omtae-wire` | Unauthorized peer connections |
| 9 | Security Headers | `omtae-api` | XSS, clickjacking, MIME sniffing |
| 10 | GCRA Rate Limiter | `omtae-api` | API abuse, denial of service |
| 11 | Path Traversal Prevention | `omtae-runtime` | Directory traversal attacks |
| 12 | Subprocess Sandbox | `omtae-runtime` | Secret leakage via child processes |
| 13 | Prompt Injection Scanner | `omtae-skills` | Malicious skill prompts |
| 14 | Loop Guard | `omtae-runtime` | Stuck agent tool loops |
| 15 | Session Repair | `omtae-runtime` | Corrupted LLM conversation history |
| 16 | Health Endpoint Redaction | `omtae-api` | Information leakage |

---

## 2. Capability-Based Security

**Source:** `omtae-types/src/capability.rs`

OMTAE uses capability-based security.  An agent can only perform actions
...
**Source:** `omtae-runtime/src/sandbox.rs`
...
**Source:** `omtae-runtime/src/audit.rs`
...
**Source:** `omtae-types/src/taint.rs`

OMTAE implements a lattice-based taint propagation model that prevents
...
**Source:** `omtae-types/src/manifest_signing.rs`
...
**Source:** `omtae-runtime/src/host_functions.rs`
...
OMTAE uses `Zeroizing<String>` from the `zeroize` crate on every field
...
**LLM Drivers** (`omtae-runtime/src/drivers/`):
...
**Channel Adapters** (`omtae-channels/src/`):
...
**Web Search** (`omtae-runtime/src/web_search.rs`):
...
**Embedding** (`omtae-runtime/src/embedding.rs`):
...
**Source:** `omtae-wire/src/peer.rs`

The OMTAE Wire Protocol (OFP) uses HMAC-SHA256 with nonce-based mutual
...
**Source:** `omtae-api/src/middleware.rs`
...
**Source:** `omtae-api/src/rate_limiter.rs`

OMTAE uses the Generic Cell Rate Algorithm (GCRA) for cost-aware API
...
**Source:** `omtae-runtime/src/subprocess_sandbox.rs`
...
**Source:** `omtae-skills/src/verify.rs`
...
**Source:** `omtae-runtime/src/loop_guard.rs`
...
**Source:** `omtae-runtime/src/session_repair.rs`
...
**Source:** `omtae-api/src/routes.rs`

OMTAE provides two health endpoints with different information levels.
